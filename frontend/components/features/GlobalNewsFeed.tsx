...
export function GlobalNewsFeed({ refreshNonce = 0 }: GlobalNewsFeedProps) {
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-sm">
            <div className="p-4 border-b border-neutral-100 dark:border-neutral-800 flex items-center justify-between bg-neutral-50/50 dark:bg-neutral-900/50">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-bold text-neutral-900 dark:text-neutral-100 uppercase tracking-tight">
                        财联社全球资讯 <span className="text-[10px] font-normal text-neutral-400 ml-1">7x24H</span>
                    </h3>
                </div>
                <button 
                    ...
                    title="刷新快讯"
                    aria-label="刷新快讯"
                    className="p-1 hover:bg-neutral-200 dark:hover:bg-neutral-800 rounded-full transition-colors disabled:opacity-50"
                >
                    <RefreshCw className={clsx("w-3.5 h-3.5 text-neutral-500", loading && "animate-spin")} />
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                <div className="p-4 space-y-6 relative">
                    {/* Vertical Line */}
                    <div className="absolute left-[21px] top-6 bottom-6 w-0.5 bg-neutral-100 dark:bg-neutral-800" />

                    {news.length === 0 && !loading ? (
                        <div className="text-center py-10">
                            <p className="text-xs text-neutral-400">暂无滚动快讯</p>
                        </div>
                    ) : (
                        news.map((item, idx) => {
                            // 格式化时间，只保留时分
                            ...
                            return (
                                <div key={uniqueKey} className="relative pl-6 group">
                                    {/* Timeline Dot */}
                                    <div className="absolute left-[3px] top-1.5 w-2 h-2 rounded-full bg-blue-600 ring-4 ring-white dark:ring-neutral-900 z-10" />
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="text-[13px] text-neutral-600 dark:text-neutral-400 leading-relaxed whitespace-pre-wrap">
                                            {item.content}
                                        </div>
                                    </div>
                                </div>
            ...
            <div className="p-3 bg-neutral-50 dark:bg-neutral-900 border-t border-neutral-100 dark:border-neutral-800 flex items-center justify-center">
