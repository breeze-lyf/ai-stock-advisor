        ...
        <div className="h-screen bg-neutral-50 dark:bg-neutral-950 flex flex-col overflow-hidden">
            ...
            <main className="flex-1 min-h-0 overflow-y-auto custom-scrollbar">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-neutral-900 dark:text-white">选股器</h1>
                        <p className="text-xs text-neutral-500 dark:text-neutral-400">基本面 · 技术面 · 预设策略</p>
                    </div>
                </div>
                                ...
                                <button
                                    ...
                                    className={`p-4 rounded-xl border text-left transition-all ${
                                        selectedStrategy === strategy.value
                                            ...
                                            : "bg-white dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 border-neutral-200 dark:border-neutral-700 hover:border-blue-300"
                                    }`}
                                >
                                    ...
                                    <p className={`text-xs ${selectedStrategy === strategy.value ? "text-blue-100" : "text-neutral-500"}`}>{strategy.description}</p>
                                </button>
                            );
                        })}
                    </div>
                ...
                {loading && (
                    <div className="flex items-center justify-center py-12">
                        ...
                        <span className="ml-3 text-neutral-500">筛选中...</span>
                    </div>
                )}
                ...
                {!loading && screenerResults.length > 0 && (
                    <div className="bg-white dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
                        <div className="px-4 py-3 border-b border-neutral-200 dark:border-neutral-700 flex justify-between items-center">
                            <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">筛选结果</span>
                            <span className="text-xs text-neutral-500">{resultCount} 只股票</span>
                        </div>
                            ...
                            <table className="w-full">
                                <thead className="bg-neutral-50 dark:bg-neutral-900">
                                    <tr>
                                        ...
                                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 uppercase">价格</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 uppercase">涨跌幅%</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 uppercase">PE</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 uppercase">PB</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 uppercase">ROE%</th>
                                        <th className="px-4 py-3 text-right text-xs font-medium text-neutral-500 uppercase">RSI</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-neutral-200 dark:divide-neutral-700">
                                    {screenerResults.map((stock) => (
                                        <tr key={stock.ticker} className="hover:bg-neutral-50 dark:hover:bg-neutral-700 transition-colors">
                                            <td className="px-4 py-3 text-sm font-medium text-neutral-900 dark:text-white">{stock.ticker}</td>
                                            <td className="px-4 py-3 text-sm text-neutral-600 dark:text-neutral-300">{stock.name}</td>
                                            <td className="px-4 py-3 text-sm text-right text-neutral-900 dark:text-white">${stock.current_price?.toFixed(2) || "--"}</td>
                                            <td className="px-4 py-3 text-sm text-right text-neutral-600 dark:text-neutral-300">{stock.pe_ratio?.toFixed(2) || "--"}</td>
                                            <td className="px-4 py-3 text-sm text-right text-neutral-600 dark:text-neutral-300">{stock.pb_ratio?.toFixed(2) || "--"}</td>
                                            <td className="px-4 py-3 text-sm text-right text-neutral-600 dark:text-neutral-300">{stock.roe?.toFixed(2) || "--"}</td>
                                            <td className="px-4 py-3 text-sm text-right text-neutral-600 dark:text-neutral-300">{stock.rsi_14?.toFixed(2) || "--"}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
