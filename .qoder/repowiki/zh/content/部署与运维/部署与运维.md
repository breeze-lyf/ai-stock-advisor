...
## 更新摘要
**所做更改**
- 新增Docker入口点脚本（entrypoint.sh）：实现了数据库迁移和Gunicorn启动的分离，提高部署可靠性和维护性
- 更新Docker配置：Dockerfile中集成入口点脚本，支持环境变量配置
- 更新生产环境部署：docker-compose.prod.yml中支持环境变量注入和健康检查
- 更新CI/CD流水线：.github/workflows/deploy.yml中支持入口点脚本的部署流程
- 更新部署脚本：scripts/deploy_compose_prod.sh中移除手动迁移步骤，改用入口点脚本自动执行
...
## 简介
本指南面向部署与运维团队，覆盖从开发环境搭建到生产部署、CI/CD 设计、监控与日志、备份与灾备、扩展性与安全加固，以及故障排除与自动化运维工具使用。项目采用前后端分离架构：后端基于 FastAPI（Python），前端基于 Next.js（TypeScript）。数据库默认使用 SQLite（开发环境），支持通过环境变量切换到PostgreSQL（生产环境）。

**重大更新**：部署基础设施已从GitHub Container Registry (GHCR) 迁移到Alibaba Cloud Container Registry (ACR)，这是生产部署基础设施的重大升级。新的CI/CD流水线采用ACR作为镜像仓库，支持云端构建+镜像拉取的轻量部署模式，显著提升部署效率和可靠性。

**新增**：完整的ACR认证配置和环境变量管理，包括ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD等敏感配置的使用。

**新增**：Docker入口点脚本（entrypoint.sh）实现了数据库迁移和Gunicorn启动的分离，这是一个重要的架构改进，提高了部署的可靠性和维护性。该脚本在容器启动时自动执行数据库迁移，然后启动Gunicorn + Uvicorn组合服务器。
...
ENV["backend/.env"]
EP["backend/scripts/entrypoint.sh"]
end
subgraph "前端"
...
**图表来源**
- [backend/app/main.py:1-169](file://backend/app/main.py#L1-L169)
...
- **更新** Alibaba Cloud Container Registry (ACR) 镜像仓库：支持后端和前端镜像托管，替代原有的GHCR配置。
- **更新** Docker Compose编排：开发环境使用本地构建，生产环境使用ACR镜像拉取。
- **更新** GitHub Actions自动化：完整的CI/CD流水线，支持云端构建、ACR镜像推送和远程部署。
- **新增** ACR认证配置：使用ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD等环境变量进行镜像仓库认证。
- **新增** 环境变量管理：统一的ACR环境变量使用，确保部署流程的一致性和安全性。
- **新增** Docker入口点脚本：entrypoint.sh，实现数据库迁移和Gunicorn启动的分离，提高部署可靠性。
- **新增** 入口点脚本集成：Dockerfile中集成入口点脚本，支持WEB_CONCURRENCY和GUNICORN_TIMEOUT环境变量配置。
...
## 架构总览
下图展示开发与生产两种运行模式的关键差异：开发模式使用本地构建和SQLite数据库，生产模式采用ACR镜像仓库和PostgreSQL数据库，支持Docker Compose编排和Nginx反向代理。**重大更新**：生产模式现采用ACR镜像仓库和Docker Compose编排，通过GitHub Actions实现云端构建和远程部署，显著提升部署效率和可靠性。

**新增**：Docker入口点脚本架构，实现了数据库迁移和服务器启动的分离，提高了部署的可靠性和维护性。
...
subgraph "生产模式"
NGINX["反向代理/Nginx"]
ACR["镜像仓库/Alibaba Cloud Container Registry"]
DCP["Docker Compose<br/>生产编排"]
BEPROD["后端服务容器<br/>Gunicorn + Uvicorn"]
DBPROD["PostgreSQL 数据库"]
REDIS["缓存/会话存储可选"]
MON["监控与日志聚合"]
GHA["GitHub Actions<br/>CI/CD自动化<br/>云端构建+镜像推送<br/>ACR认证配置"]
EP["Docker 入口点脚本<br/>entrypoint.sh<br/>数据库迁移 + Gunicorn启动"]
GHA --> ACR
ACR --> DCP
DCP --> NGINX
NGINX --> BEPROD
...
### 生产环境部署策略
- 镜像仓库与编排
  - 使用Alibaba Cloud Container Registry (ACR)托管后端和前端镜像，替代原有的GHCR配置。
...
### CI/CD 流水线设计
- **更新** GitHub Actions自动化部署（ACR + Docker Compose模式）
  - **三阶段部署流程**：
    1. **云端构建阶段**：在GitHub Actions环境中构建后端和前端镜像，使用Buildx加速构建
    2. **镜像推送阶段**：将构建好的镜像推送到ACR，使用ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD进行认证，支持latest和SHA标签
    3. **远程部署阶段**：通过SCP传输编排文件和部署脚本到服务器，使用SSH执行部署
  - **镜像管理**：后端和前端分别构建独立镜像，使用${{ secrets.ACR_REGISTRY }}/ai-stock-advisor/stock-backend格式的镜像标签
  - **部署自动化**：服务器端仅执行docker pull和compose up，避免本地构建压力
  - **部署优势**：云端构建利用GitHub Actions的强大计算资源，服务器端仅负责拉取和启动。
  - **新增** **ACR认证配置**：使用docker/login-action@v3进行ACR认证，配置registry、username、password参数。
  - **新增** **环境变量管理**：在服务器端通过export ACR_REGISTRY="${{ secrets.ACR_REGISTRY }}"设置环境变量。
- 传统部署方式
  - 本地Docker Compose部署：使用scripts/deploy.sh脚本，支持容器化部署与数据库迁移。
  - 远程部署：使用scripts/deploy_compose_prod.sh脚本，支持ACR镜像拉取和数据库迁移。
...
- **新增** Docker入口点脚本集成
  - entrypoint.sh在容器启动时自动执行数据库迁移
  - 支持WEB_CONCURRENCY和GUNICORN_TIMEOUT环境变量配置
  - 通过Dockerfile集成，确保每次启动都执行迁移
...
**章节来源**
- [backend/app/main.py:160-169](file://backend/app/main.py#L160-L169)
...
- [backend/app/main.py:1-169](file://backend/app/main.py#L1-L169)
- [backend/app/core/database.py:1-69](file://backend/app/core/database.py#L1-L69)
- [backend/Dockerfile:25-34](file://backend/Dockerfile#L25-L34)
...
- **更新** 镜像安全**
  - ACR镜像访问控制，使用ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD进行认证。
  - 镜像扫描和漏洞检测集成到CI/CD流程。
  - **新增** ACR认证配置：使用docker/login-action@v3进行ACR认证，确保镜像仓库访问安全。

**章节来源**
- [backend/app/main.py:114-135](file://backend/app/main.py#L114-L135)
- [backend/app/core/security.py:1-26](file://backend/app/core/security.py#L1-L26)
...
- **更新** GitHub Actions自动化（ACR + Docker Compose模式）
  - deploy.yml：完整的三阶段CI/CD流水线，支持ACR镜像仓库和远程部署。
  - 云端构建：在GitHub Actions环境中进行镜像构建，利用云端资源。
  - 镜像管理：支持latest和SHA标签，便于版本控制和回滚。
  - **新增** ACR认证配置：使用ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD进行镜像仓库认证。
  - **新增** 环境变量管理：在服务器端通过export ACR_REGISTRY="${{ secrets.ACR_REGISTRY }}"设置环境变量。
...
- **新增** Docker Compose部署脚本
  - deploy_compose_prod.sh：生产环境部署脚本，支持ACR镜像拉取和数据库迁移。
  - deploy.sh：本地Docker Compose部署脚本。
  - **新增** remote_deploy.sh：完整的远程部署脚本，支持rsync文件同步和自动构建。
- **新增** 多环境部署配置
  - docker-compose.yml：开发环境本地构建配置。
  - docker-compose.prod.yml：生产环境ACR镜像配置。
- **新增** 监控与日志配置
  - Grafana仪表板：提供AI股票分析系统的可视化监控面板。
  - Loki日志聚合：支持大规模日志收集和查询。
  - Promtail日志收集：提供高效的日志采集和转发功能。
- **新增** Docker入口点脚本
  - entrypoint.sh：实现数据库迁移和Gunicorn启动的分离，提高部署可靠性。
  - Dockerfile中集成入口点脚本，支持环境变量配置。
  - 支持WEB_CONCURRENCY和GUNICORN_TIMEOUT环境变量。
- **新增** 入口点脚本集成
  - Dockerfile第31-33行：RUN chmod +x /app/scripts/entrypoint.sh
  - Dockerfile第33行：ENTRYPOINT ["/app/scripts/entrypoint.sh"]
  - 支持容器启动时自动执行数据库迁移。
- **新增** 部署流程优化
  - 移除deploy_compose_prod.sh中的手动迁移步骤，改用入口点脚本自动执行。
  - 提高部署流程的可靠性和一致性。
...
- 关键耦合点
  ...
  - **新增** ACR镜像仓库与CI/CD流程耦合，实现自动化镜像管理。
  - **新增** Docker Compose与生产环境耦合，实现服务编排和资源管理。
  - **更新** GitHub Actions与ACR镜像仓库耦合，实现云端构建和远程部署。
  - **新增** ACR认证配置与部署流程耦合，提供安全的镜像仓库访问。
  - **新增** Docker入口点脚本与部署流程耦合，实现数据库迁移和服务器启动的分离。
  - **新增** 环境变量与入口点脚本耦合，支持WEB_CONCURRENCY和GUNICORN_TIMEOUT配置。
...
NEONMIG["Neon迁移工具"] --> DB
ACR["Alibaba Cloud Container Registry"] --> DCP["Docker Compose"]
GHA["GitHub Actions"] --> ACR
GHA --> DCP
GHA --> ACR_AUTH["ACR认证配置<br/>ACR_REGISTRY/ACR_USERNAME/ACR_PASSWORD"]
DCP --> NGINX["Nginx反向代理"]
MON["监控与日志"] --> GRAFANA["Grafana仪表板"]
MON --> LOKI["Loki日志聚合"]
MON --> PROMTAIL["Promtail日志收集"]
EP["Docker入口点脚本<br/>entrypoint.sh"] --> DB
EP --> GUNICORN["Gunicorn + Uvicorn服务器"]
ENV["环境变量<br/>WEB_CONCURRENCY/GUNICORN_TIMEOUT"] --> EP
```
...
- API 性能
  ...
  - **新增** 入口点脚本优化：通过WEB_CONCURRENCY环境变量控制worker数量，提高并发处理能力。
  - **新增** 超时配置：通过GUNICORN_TIMEOUT环境变量设置超时时间，避免长时间阻塞。
- 前端性能
...
- **更新** ACR部署性能优化
  ...
  - **新增** ACR认证优化：使用docker/login-action@v3进行ACR认证，提升镜像拉取效率。
  - **新增** 环境变量管理：统一的ACR环境变量使用，确保部署流程的一致性和安全性。
  - **新增** 入口点脚本性能：容器启动时自动执行数据库迁移，避免重复迁移开销。
...
- **更新** ACR镜像拉取问题
  - 镜像不存在：检查ACR仓库权限和镜像标签。
  - 认证失败：检查ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD配置。
  - 网络超时：检查网络连通性和ACR仓库可用性。
- **更新** Docker Compose部署问题
  - 服务启动失败：检查容器日志和依赖服务状态。
  - 端口冲突：确认宿主机端口未被占用。
  - 环境变量缺失：检查.env文件和环境变量注入。
- **新增** Docker入口点脚本问题
  - 迁移失败：检查数据库连接、迁移脚本语法和权限。
  - Gunicorn启动失败：检查WEB_CONCURRENCY和GUNICORN_TIMEOUT配置。
  - 权限问题：确认entrypoint.sh具有执行权限。
  - 环境变量未设置：检查WEB_CONCURRENCY和GUNICORN_TIMEOUT环境变量。
- **新增** ACR认证问题
  - docker login失败：检查ACR_REGISTRY格式和ACR_USERNAME、ACR_PASSWORD配置。
  - 镜像拉取权限不足：确认ACR用户权限和镜像仓库访问策略。
  - 环境变量未设置：检查ACR_REGISTRY环境变量是否正确导出。
- **新增** 数据库迁移问题
  - alembic迁移失败：检查数据库连接、迁移脚本语法和权限。
  - Neon迁移中断：检查源数据库连接、目标数据库权限和网络连通性。
  - LLM数据迁移失败：检查目标表结构和数据类型兼容性。
- **更新** GitHub Actions部署问题（ACR模式）
  - 镜像构建失败：检查Dockerfile和依赖安装。
  - ACR推送失败：检查ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD权限和仓库可见性。
  - SCP传输失败：检查服务器SSH密钥和目标路径权限。
  - SSH连接失败：检查服务器IP、用户名、私钥配置。
  - 依赖安装超时：检查网络连接和镜像源配置。
  - 服务器端最终化失败：检查后端依赖安装和Python环境。
  - **新增** ACR认证失败：检查docker/login-action@v3配置和secrets设置。
  - **新增** 环境变量问题：检查ACR_REGISTRY环境变量导出和使用。
  - **新增** 数据库迁移失败：检查alembic配置、数据库连接和迁移权限。
  - **新增** 入口点脚本执行失败：检查容器日志中的迁移错误信息。
- **新增** 传统部署问题
...
## 结论
本指南提供了从开发到生产的完整运维路径：以 scripts/start.sh 快速起步，以 init_db.py 与 migrate_db.py 管理数据库生命周期，以 alembic 和迁移工具实现数据库结构演进，以 .env 与配置系统实现环境隔离，以CI/CD保障交付质量，以监控与日志实现可观测性，以安全加固与灾备提升韧性。

**重大更新**：部署基础设施已从GitHub Container Registry (GHCR) 迁移到Alibaba Cloud Container Registry (ACR)，这是生产部署基础设施的重大升级。新的CI/CD流水线采用ACR作为镜像仓库，支持云端构建+镜像拉取的轻量部署模式，显著提升了部署效率和可靠性。

**新增**：完整的ACR认证配置和环境变量管理，包括ACR_REGISTRY、ACR_USERNAME、ACR_PASSWORD等敏感配置的使用。**Docker Compose编排**为生产环境提供了稳定的服务管理。**新增**：完整的监控与日志系统，包括Grafana仪表板、Loki日志聚合和Promtail日志收集器。**新增**：Docker入口点脚本（entrypoint.sh）实现了数据库迁移和Gunicorn启动的分离，这是一个重要的架构改进，提高了部署的可靠性和维护性。

建议在生产中采用ACR镜像仓库、Docker Compose编排、多可用区部署与自动化运维工具，持续优化性能与可靠性。同时，充分利用入口点脚本的自动迁移功能，简化部署流程并提高系统稳定性。

## 附录
- 快速对照表
  - 开发：使用 scripts/start.sh；SQLite；.env 示例；前端本地开发服务器。
  - 生产：ACR镜像仓库；Docker Compose编排；PostgreSQL数据库；密钥管理；监控与日志；备份与灾备；**ACR + Docker Compose部署**；**alembic数据库迁移**；**ACR认证配置**；**Docker入口点脚本**。
- 参考文件
  - 技术栈与开发规范：README.md
  ...
  - **更新** CI/CD自动化：.github/workflows/deploy.yml（ACR模式）
  - **新增** ACR认证配置：docker/login-action@v3认证步骤
  - **新增** 环境变量管理：ACR_REGISTRY环境变量导出
  ...
  - **新增** Docker入口点脚本：backend/scripts/entrypoint.sh
  - **新增** Dockerfile集成：backend/Dockerfile中入口点脚本集成
  - **新增** 环境变量配置：WEB_CONCURRENCY、GUNICORN_TIMEOUT