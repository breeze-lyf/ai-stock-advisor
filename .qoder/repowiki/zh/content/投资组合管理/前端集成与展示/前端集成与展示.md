...
- [frontend/components/features/stock-detail/shared.tsx](file://frontend/components/features/stock-detail/shared.tsx)
...
- **新增** AI Verdict 组件获得增强的加载状态和时间戳格式化功能
...
**更新** 本版本重点介绍了全新的仪表板组件架构、hooks 驱动的状态管理和基于缓存的资源管理系统，这些改进显著提升了应用的模块化程度和用户体验。**新增** AI Verdict 组件获得了增强的加载状态和时间戳格式化功能，提供更丰富的用户反馈和更精确的时间显示。

## 项目结构
前端采用 Next.js 应用程序目录结构，页面组件位于 app 下，UI 组件位于 components/ui，全局上下文与布局位于 app 与 context，工具函数位于 lib。**新增** 仪表板功能位于 features/dashboard 目录，包含组件、hooks 和 API 集成。**新增** 股票详情功能位于 components/features/stock-detail 目录，包含 AI Verdict 组件及其相关类型和共享工具。
...
UDSD["useDashboardStockDetailData.ts<br/>股票详情数据钩子"]
end
subgraph "上下文"
...
SH["shared.tsx<br/>共享工具函数"]
TY["types.ts<br/>类型定义"]
end
subgraph "股票详情功能"
SD["StockDetail.tsx<br/>股票详情编排层"]
AV["AIVerdict.tsx<br/>AI智能判研指标<br/>增强加载状态"]
end
L --> H
H --> DS
DS --> ATC
...
- [frontend/components/features/stock-detail/AIVerdict.tsx:1-566](file://frontend/components/features/stock-detail/AIVerdict.tsx#L1-L566)
- [frontend/components/features/stock-detail/shared.tsx:1-132](file://frontend/components/features/stock-detail/shared.tsx#L1-L132)
- [frontend/components/features/stock-detail/types.ts:1-102](file://frontend/components/features/stock-detail/types.ts#L1-L102)
- [frontend/features/dashboard/hooks/useDashboardStockDetailData.ts:1-79](file://frontend/features/dashboard/hooks/useDashboardStockDetailData.ts#L1-L79)
...
- **股票详情编排层（StockDetail）**：**新增** 作为股票详情页面的编排容器，协调各个子组件的渲染和状态管理。
- **AI 智能判研指标（AIVerdict）**：**新增** 增强的 AI 分析结果显示组件，包含改进的加载状态和时间戳格式化功能。
- **共享工具函数（shared.tsx）**：**新增** 提供跨组件复用的工具函数，包括价格清洗、货币符号判断等。
- **类型定义（types.ts）**：**新增** 为所有子组件提供严格的 Props 接口契约，确保类型安全。
...
- [frontend/features/dashboard/hooks/useDashboardRouteState.ts:1-73](file://frontend/features/dashboard/hooks/useDashboardRouteState.ts#L1-L73)
- [frontend/features/dashboard/hooks/dashboardCache.ts:1-91](file://frontend/features/dashboard/hooks/dashboardCache.ts#L1-L91)
- [frontend/components/features/StockDetail.tsx:1-189](file://frontend/components/features/StockDetail.tsx#L1-L189)
- [frontend/components/features/stock-detail/AIVerdict.tsx:1-566](file://frontend/components/features/stock-detail/AIVerdict.tsx#L1-L566)
- [frontend/components/features/stock-detail/shared.tsx:1-132](file://frontend/components/features/stock-detail/shared.tsx#L1-L132)
- [frontend/components/features/stock-detail/types.ts:1-102](file://frontend/components/features/stock-detail/types.ts#L1-L102)
...
## 架构总览
前端通过认证上下文与本地存储维护用户会话；**新增** 仪表板架构采用 hooks 驱动的状态管理，通过缓存系统实现数据的高效管理；页面组件通过 API 客户端获取投资组合与分析数据；UI 组件负责渲染与交互；设置页用于解锁无限分析能力与切换数据源。**新增** Suspense 提供统一的加载状态管理。**新增** 股票详情功能通过 StockDetail 编排层协调各个子组件，其中 AIVerdict 组件提供增强的加载状态和时间戳格式化功能。
...
StockDetail["StockDetail.tsx<br/>股票详情编排层"]
AIVerdict["AIVerdict.tsx<br/>AI智能判研指标<br/>增强加载状态"]
Shared["shared.tsx<br/>共享工具函数"]
...
- **useDashboardStockDetailData**：**新增** 管理股票详情页面的数据获取和缓存。
...
UDSD["useDashboardStockDetailData<br/>股票详情数据管理"]
Hooks --> UDA
Hooks --> UDP
Hooks --> UDRS
Hooks --> UCR
...
- [frontend/features/dashboard/hooks/useDashboardStockDetailData.ts:17-79](file://frontend/features/dashboard/hooks/useDashboardStockDetailData.ts#L17-L79)
...
### 股票详情编排层（StockDetail）- **新增**
职责与流程
- **全局状态管理**：管理滚动检测、图层切换、刷新状态等全局状态。
- **数据协调**：通过 useDashboardStockDetailData 协调股票历史数据和分析历史数据的获取。
- **组件编排**：将各个子组件（StickyBar、HeaderIdentity、MarketAnalysis、AIVerdict、TechnicalInsights、FundamentalCard、NewsFeed）进行编排和渲染。
- **交互处理**：处理刷新、滚动检测等用户交互事件。

```mermaid
flowchart TD
Start(["StockDetail 初始化"]) --> State["初始化全局状态<br/>- refreshing<br/>- showBb/showRsi/showMacd<br/>- isScrolled"]
State --> Hooks["useDashboardStockDetailData<br/>获取分析历史和市场数据"]
Hooks --> Props["准备子组件 Props<br/>- selectedItem<br/>- aiData<br/>- analysisHistory<br/>- historyData"]
Props --> Render["渲染各个子组件"]
Render --> Scroll["滚动检测 Effect<br/>监听滚动事件"]
Render --> Refresh["刷新处理<br/>调用 refreshStock API"]
```

**图表来源**
- [frontend/components/features/StockDetail.tsx:53-188](file://frontend/components/features/StockDetail.tsx#L53-L188)

**章节来源**
- [frontend/components/features/StockDetail.tsx:1-189](file://frontend/components/features/StockDetail.tsx#L1-L189)

### AI 智能判研指标（AIVerdict）- **新增**
职责与流程
- **增强的加载状态**：提供更丰富的加载状态反馈，包括诊断中状态和进度提示。
- **时间戳格式化**：使用 date-fns 的 formatDistanceToNow 和 format 函数提供本地化的时差显示和日期格式化。
- **模拟交易集成**：集成模拟交易功能，支持一键创建模拟交易订单。
- **交易轴可视化**：提供可视化的交易执行区间展示，包括止损、建仓、止盈等关键价位。
- **诊断逻辑展示**：以 Markdown 格式展示 AI 的诊断研判逻辑和建议。

**更新** 增强的加载状态和时间戳格式化功能
- **加载状态增强**：按钮状态从简单的启用/禁用扩展为更详细的诊断中状态，包括动画脉冲效果和进度提示。
- **时间戳格式化**：使用 zhCN 本地化配置提供中文友好的时间显示，支持相对时间（如"5分钟前"）和绝对日期格式。
- **模拟交易状态管理**：提供创建中、已创建等状态反馈，改善用户体验。

```mermaid
sequenceDiagram
participant U as "用户"
participant AV as "AIVerdict"
participant B as "Button"
participant API as "API"
U->>AV : 点击"开启深度诊断"
AV->>B : 更新按钮状态为"诊断中..."
AV->>B : 显示进度提示"深度思考约需 2-3 分钟，请稍后"
AV->>API : 发送分析请求
API-->>AV : 返回分析结果或错误
alt 分析成功
API-->>AV : 返回分析数据
AV->>AV : 格式化时间戳<br/>使用 formatDistanceToNow
AV->>AV : 渲染增强的加载状态
else 分析失败
AV->>AV : 进入轮询恢复模式
loop 最多5次重试
AV->>API : getLatestAnalysis(ticker)
API-->>AV : 缓存数据或错误
end
alt 恢复成功
AV->>AV : 格式化时间戳并渲染
...
**图表来源**
- [frontend/components/features/stock-detail/AIVerdict.tsx:46-59](file://frontend/components/features/stock-detail/AIVerdict.tsx#L46-L59)
- [frontend/components/features/stock-detail/AIVerdict.tsx:360-366](file://frontend/components/features/stock-detail/AIVerdict.tsx#L360-L366)

**章节来源**
- [frontend/components/features/stock-detail/AIVerdict.tsx:1-566](file://frontend/components/features/stock-detail/AIVerdict.tsx#L1-L566)

### 共享工具函数（shared.tsx）- **新增**
职责与流程
- **价格清洗**：提供统一的价格格式化函数，将 null/undefined/NaN 统一渲染为 "--"。
- **货币符号判断**：根据股票代码后缀判断货币符号（美元或人民币）。
- **RSI 进度条颜色**：根据 RSI 数值返回相应的颜色类名。
- **引用高亮系统**：提供引用标签组件和高亮闪烁动画功能。
- **Markdown 渲染器**：封装带引用解析的 Markdown 渲染器，支持自定义组件样式。

**更新** 增强的工具函数
- **价格清洗增强**：提供更健壮的价格格式化，支持小数点位数控制。
- **货币符号判断**：支持更多股票市场的货币符号识别。
- **引用解析增强**：改进 Markdown 渲染器，支持更复杂的引用标记解析。
...
### 类型定义（types.ts）- **新增**
职责与流程
- **AI 数据接口**：定义完整的 AIData 结构，包括情感分数、技术分析、行动建议、价格预测等字段。
- **组件 Props 接口**：为各个子组件提供严格的 Props 接口契约，确保类型安全。
- **数据结构标准化**：提供统一的数据结构定义，便于组件间的数据传递和验证。

**更新** 增强的类型定义
- **AIData 接口增强**：扩展 AI 数据结构，包含更多分析维度和元数据字段。
- **Props 接口完善**：为每个子组件提供更精确的 Props 类型定义。
...
- **新增** 股票详情数据绑定：通过 useDashboardStockDetailData 实现股票历史数据和分析历史数据的缓存管理。
- 实时更新策略：
...
- [frontend/features/dashboard/hooks/useDashboardStockDetailData.ts:50-79](file://frontend/features/dashboard/hooks/useDashboardStockDetailData.ts#L50-L79)
...
- **新增** 股票详情组件定制：
  - 通过 StockDetail 的 Props 接口自定义各个子组件的渲染。
  - 通过 AIVerdict 的 Props 接口自定义 AI 分析结果的展示。
  - 通过 shared.tsx 的工具函数自定义数据格式化规则。
- 功能扩展：
  ...
  - **新增** 类型安全：通过 types.ts 的接口定义确保组件间的数据传递安全。
...
- [frontend/features/dashboard/hooks/dashboardCache.ts:64-91](file://frontend/features/dashboard/hooks/dashboardCache.ts#L64-L91)
- [frontend/components/features/StockDetail.tsx:32-52](file://frontend/components/features/StockDetail.tsx#L32-L52)
- [frontend/components/features/stock-detail/AIVerdict.tsx:28-36](file://frontend/components/features/stock-detail/AIVerdict.tsx#L28-L36)
- [frontend/components/features/stock-detail/shared.tsx:15-25](file://frontend/components/features/stock-detail/shared.tsx#L15-L25)
- [frontend/components/features/stock-detail/types.ts:11-36](file://frontend/components/features/stock-detail/types.ts#L11-L36)
...
- **新增** 股票详情依赖：date-fns 的 zhCN 本地化配置，支持中文时间显示。
...
- **新增** 类型系统：通过 TypeScript 接口定义确保组件间的数据传递安全。
...
- [frontend/components/features/stock-detail/AIVerdict.tsx:21-22](file://frontend/components/features/stock-detail/AIVerdict.tsx#L21-L22)
...
- **新增** 股票详情优化：通过 useDashboardStockDetailData 实现股票历史数据和分析历史数据的缓存管理。
...
- **新增** 股票详情问题：检查 useDashboardStockDetailData 的数据获取和缓存状态。
- **新增** AIVerdict 组件问题：验证加载状态的正确显示，检查时间戳格式化功能是否正常工作。
...
- 数据不更新：确认 fetchData 是否在登录成功后执行；检查市场状态定时器是否正常运行；**新增** 验证 Suspense 加载状态是否正确显示；**新增** 检查 hooks 状态管理是否正常工作；**新增** 验证 AIVerdict 组件的增强加载状态功能。
- **新增** Suspense 相关问题：检查 Dashboard 组件的 Suspense 包装是否正确；验证 fallback 组件的样式和内容。
- **新增** 响应式布局问题：检查 AnalysisTabContainer 在不同屏幕尺寸下的表现。
- **新增** 时间格式化问题：检查 zhCN 本地化配置是否正确加载，验证 formatDistanceToNow 和 format 函数的使用。
...
## 结论
该前端实现以页面组件为核心，结合认证上下文与 UI 组件库，构建了完整的投资组合展示与交互体系。**更新** 通过引入全新的仪表板组件架构、hooks 驱动的状态管理和基于缓存的资源管理系统，显著提升了应用的模块化程度、性能和用户体验。**新增** 的 DashboardShell、AnalysisTabContainer 等组件提供了更加灵活和可扩展的仪表板解决方案；hooks 系统实现了更好的状态管理和代码复用；缓存系统优化了数据获取效率。**新增** 的 StockDetail 编排层和 AIVerdict 组件进一步增强了股票详情页面的功能性和用户体验。**新增** 的增强加载状态和时间戳格式化功能为用户提供了更丰富、更直观的反馈信息。通过本地状态管理与定时器策略，实现了稳定的数据绑定与实时更新；通过设置页与 API Key 管理，提升了用户体验与功能可用性。建议持续关注长列表性能与错误边界处理，进一步增强可访问性与国际化支持。
...
- 术语
  ...
  - **新增** 股票详情编排层：StockDetail 作为股票详情页面的协调器。
  - **新增** AI 智能判研指标：AIVerdict 提供增强的 AI 分析结果显示。
  - **新增** 共享工具函数：shared.tsx 提供跨组件复用的工具函数。
  - **新增** 类型定义：types.ts 提供严格的类型安全保证。
  - **新增** 增强加载状态：AIVerdict 的诊断中状态和进度提示。
  - **新增** 时间戳格式化：使用 zhCN 本地化配置的中文时间显示。
- 参考路径
  ...
  - **新增** 股票详情编排层：[frontend/components/features/StockDetail.tsx:1-189](file://frontend/components/features/StockDetail.tsx#L1-L189)
  - **新增** AI 智能判研指标：[frontend/components/features/stock-detail/AIVerdict.tsx:1-566](file://frontend/components/features/stock-detail/AIVerdict.tsx#L1-L566)
  - **新增** 共享工具函数：[frontend/components/features/stock-detail/shared.tsx:1-132](file://frontend/components/features/stock-detail/shared.tsx#L1-L132)
  - **新增** 类型定义：[frontend/components/features/stock-detail/types.ts:1-102](file://frontend/components/features/stock-detail/types.ts#L1-L102)
