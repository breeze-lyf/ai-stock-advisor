...
const TYPE_COLORS: Record<string, string> = {
    ...
    macro: "bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-700/20 dark:text-slate-300 dark:border-slate-700",
...
const IMPACT_LEVEL_CONFIG = {
    ...
    low: {
        ...
        tag: "bg-slate-50 text-slate-600 border-slate-200 dark:bg-slate-700/20 dark:text-slate-300 dark:border-slate-700",
        card: "bg-slate-50/60 border-slate-100 dark:bg-slate-800/40 dark:border-slate-700/60",
        label: "低影响",
    },
} as const;
    ...
    return (
        <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-6 py-3 border-b border-slate-100 dark:border-zinc-800 flex items-center gap-2">
                ...
                <span className="text-[10px] text-slate-400 ml-1">— 未来30天内可能影响判断的关键事件</span>
            </div>
                ...
                <div className="relative pt-2 pb-4 mb-5">
                    <div className="relative h-4 mb-2">
                        {[0, 7, 14, 21, 30].map((day) => (
                            ...
                            >
                                <span className="text-[9px] font-medium text-slate-400 mono">
                                    {day === 0 ? "D+0" : `D+${day}`}
                                </span>
                            </div>
                        ))}
                    ...
                    <div className="relative h-16">
                        <div className="absolute left-0 right-0 top-1/2 -translate-y-1/2 h-px bg-slate-200 dark:bg-zinc-700" />
                        {[0, 7, 14, 21, 30].map((day) => (
                            <div
                                key={day}
                                className="absolute top-1/2 -translate-y-1/2 w-px h-1.5 bg-slate-300 dark:bg-zinc-600"
                                style={{ left: `${(day / 30) * 100}%` }}
                            />
                        ))}
                                ...
                                >
                                    <div className="flex flex-col items-center">
                                        ...
                                        <div className="mt-1.5 text-center">
                                            <div className="text-[10px] font-bold text-slate-900 dark:text-slate-200 leading-tight whitespace-nowrap">
                                                {catalyst.event}
                                            </div>
                                ...
                                <div className="flex items-start gap-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-2 flex-wrap mb-1">
                                            ...
                                            <span className="ml-auto text-[10px] mono text-slate-500 dark:text-slate-400">
