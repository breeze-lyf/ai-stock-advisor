...
2026-05-17: 撤回"加同业对比模块"建议 — 我曾建议在 [prototype-analyst-view.html](frontend/public/prototype-analyst-view.html) 加"为什么是 GOOGL 不是 MSFT"的 peer 对比行。审计数据源后撤回：yfinance 的 sector 字段粒度不匹配用户认知中的 peer、peer 数据要批量拉行情、解释"为什么选 X"需要对 peer 也做 AI 分析（成本 5×）。教训：产品话术前必须先核查数据可得性。
2026-05-17: 识别后端 AI 输出半成品链路 — prompts.py 已要求 LLM 返回 confidence_breakdown / key_assumptions / catalysts 三个结构化字段，application/analysis/mappers.py 也在解析，但 **AnalysisReport ORM 没列、AnalysisResponse schema 没暴露**。Why: 这是 CLAUDE.md 第 3.2 条禁止的"链路一截"——backend → openapi → 前端类型 → 渲染应该同步完成。How to apply: 任何使用这些字段的前端模块都需要先补完链路；prototype-analyst-view.html 里的"信心三维度""关键假设""催化剂"模块在生产环境前都要标注此依赖。
2026-05-17: prototype-analyst-view.html 完成两轮内容/视觉优化 — 删除"情绪偏差/信心"双标、修正"距 20MA -2.1%"的反向误导、合并 11 张冗余卡片为 6 张、价格轴从 2px → 16px、催化剂改为真正横向时间轴、行业敞口改双轨对比。视觉风格保持 v1（圆角 + 渐变 + 高饱和卡片），不切换到 Bloomberg 终端风（用户明确要求保持统一）。
