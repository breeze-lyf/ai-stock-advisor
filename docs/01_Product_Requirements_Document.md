...
| V4.1 | 2026-04-02 | GitHub Copilot | 按当前仓库实现情况校准：补充当前落地状态、修正技术栈/AI模型/版本规划与实施边界 |

---

## 0. 2026年4月当前落地状态说明

本次修订基于当前仓库代码、README、OpenAPI 与目录结构进行校准，目标是让 PRD 同时保留产品规划视角与当前实现边界，避免“计划态”和“运行态”混淆。

**当前系统形态：**

| 维度 | 当前状态 |
|------|----------|
| 产品形态 | Web 前端 + FastAPI 后端 + Taro 移动端并行维护 |
| 运行方式 | 本地开发以应用本机运行 + Neon Postgres 为主；生产推荐使用 GHCR 镜像 + `docker-compose.prod.yml` |
| 数据层 | PostgreSQL/Neon 为主库，Redis 用于缓存、去重与通知辅助；SQLite 已不再作为默认数据库 |
| AI能力 | 已支持系统内置模型与用户自定义模型（BYOK）共存；当前默认内置模型为 DashScope / `qwen3.5-plus` |
| 已上线核心模块 | 注册登录、持仓管理、个股 AI 诊断、组合 AI 分析、宏观雷达、通知中心、设置中心、模拟交易基础流程 |
| 当前约束 | 移动端模拟交易仍以“创建 + 列表查看”为主；Truth Tracker 字段与基础追踪已落库，但完整胜率/复盘运营面板仍需继续打磨 |
| 大陆可用性优化 | 已增加代理可达性检测、AkShare 国内域名直连绕过、以及 Tavily 受限时的财联社降级链路 |
...
| 技术 | 选型理由 | 替代方案对比 |
|------|----------|--------------|
| **Next.js 16（App Router）** | SSR/路由/构建一体化，适合金融信息页与受保护页面混合场景 | 纯 SPA 方案首屏与 SEO 能力较弱 |
| **React 19** | 支持最新组件模式，便于维持复杂诊断页与设置页交互 | 旧版本在新生态兼容性上成本更高 |
| **TypeScript 5** | 类型安全、接口契约明确、便于与 OpenAPI 类型联动 | JavaScript 在大型演进中维护成本更高 |
| **Tailwind CSS 4** | 原子化样式与设计系统收敛效率高，适合快速迭代业务界面 | 传统样式方案在多人协作下更易分叉 |
| **Radix UI + 自定义业务组件** | 底层交互稳定，可在不引入重型设计系统的前提下构建金融业务组件 | 全量 UI 框架定制成本更高 |
| **Axios + OpenAPI Types** | 前后端接口层已形成显式 API 模块与类型生成链路 | 纯手写请求层容易产生契约漂移 |
...
| 服务端数据 | Axios + 领域 API 模块 + 局部页面状态 | 当前仓库已按功能域拆分请求层，复杂度可控，便于精细处理超时与鉴权刷新 |
...
| 技术 | 选型理由 | 替代方案对比 |
|------|----------|--------------|
| **Python 3.10+** | AI生态丰富、开发效率高，兼容当前仓库运行与脚本体系 | Go: 性能好但AI库少；Node: 异步复杂 |
| **FastAPI** | 异步支持、自动文档、类型检查 | Flask: 同步阻塞；Django: 过重 |
| **SQLAlchemy 2.0** | 异步ORM、成熟稳定 | Tortoise ORM: 生态较小 |
| **Neon Postgres / PostgreSQL** | 当前主数据库方案，支持云托管、异步驱动与生产部署一致性 | MySQL: JSON支持弱；MongoDB: 无事务 |
| **Redis** | 用于缓存、通知去重、辅助运行时状态；不是唯一数据源 | Memcached: 功能单一 |
| **Alembic** | 数据库迁移管理、与SQLAlchemy集成 | Django migrations: 不适用 |

**当前实现补充说明：**

- `DATABASE_URL` 已调整为必填环境变量，仓库默认不再使用 SQLite 作为运行时主库。
- 调度体系通过 FastAPI lifespan 启动常驻后台协程，而不是额外拆分独立任务服务。
- 系统启动时会同步内置 AI Provider/Model 注册信息，并执行代理环境可达性与降级处理。
...
graph TB
    subgraph "模型能力矩阵"
        A[系统默认可用] --> A1[DashScope / qwen3.5-plus]
        A --> A2[用户无需自配即可使用]
        
        B[高推理/兼容回退] --> B1[SiliconFlow / DeepSeek 系列]
        B --> B2[支持更长思考型响应]
        
        C[扩展接入] --> C1[用户自定义 Base URL + API Key + Model ID]
        C --> C2[兼容 BYOK 与 OpenAI-Compatible 路由]
    end
```
...
| 场景 | 推荐模型 | 理由 |
|------|----------|------|
| 登录即用的默认诊断 | DashScope / `qwen3.5-plus` | 当前仓库已内置注册，用户开箱即用 |
| 高推理与兼容回退 | SiliconFlow / DeepSeek 系列 | 适合承接更重的推理场景与供应商回退 |
| 自定义供应商接入 | 用户自定义模型（BYOK） | 支持通过 `base_url + api_key + model_id` 接入私有或第三方模型 |
...
graph TB
    ...
    D --> D1[DeepSeek-R1 / DeepSeek-V3]
    
    E --> E1[Gemini / DeepSeek / 其他兼容模型]
...
|--------|----------|----------|--------|
| DashScope | `https://coding.dashscope.aliyuncs.com/v1` | `qwen3.5-plus` | 当前系统内置默认 |
| SiliconFlow | `https://api.siliconflow.cn/v1` | DeepSeek-R1、DeepSeek-V3 等兼容模型 | 系统兼容回退 |
| 用户自定义兼容供应商 | 用户自填 | 取决于用户配置 | 按用户偏好与可用性生效 |

**当前实现说明：**

- 内置公开系统模型目前以 `qwen3.5-plus` 为主。
- 其他厂商模型主要通过用户自定义模型配置接入，而不是全部以系统公共模型形式开放。
- 单次个股分析仍然只调用一个最终选定模型，但会同时注入技术面、新闻、宏观、基本面与历史分析上下文。
...
截至 2026年4月，仓库内已实现 `membership_tier` 与部分功能开关基础能力，FREE/PRO 的产品分层逻辑已有数据模型承载；支付、订阅扣费与 PRO+ 的完整商业闭环仍属于后续规划。
...
| 模块 | 功能 | 当前状态 | 说明 |
|------|------|----------|------|
| 用户系统 | 邮箱注册/登录、密码修改、设置中心 | 已完成 | Web 与移动端均有入口 |
| 持仓管理 | 添加/编辑/删除持仓、组合概览 | 已完成 | 支持持仓刷新、摘要与分析入口 |
| AI诊断 | 个股深度诊断、诊断历史、交易决策轴 | 已完成 | 前端已支持长超时请求与结构化结果展示 |
| 组合分析 | 组合级 AI 报告 | 已完成 | 作为独立分析路径存在 |
| 数据服务 | A股/美股行情、技术指标、历史行情 | 已完成 | 以 PostgreSQL/缓存表承接展示数据 |
| 宏观雷达 | 宏观主题、快讯、整点摘要、影响分析 | 已完成 | 已具备调度、持久化与通知链路 |
| 通知系统 | 飞书通知、去重、宏观与价格类提醒 | 已完成 | 已接入用户通知偏好开关 |
| AI模型管理 | 系统内置模型、用户自定义模型、连通性测试 | 已完成 | 已支持 BYOK 与默认模型切换 |
| 模拟交易 | 创建、列表查看、基础监控 | 部分完成 | Web 端较完整，移动端当前以创建与列表为主 |
| Truth Tracker | MAE/MFE 等追踪字段与基础数据沉淀 | 部分完成 | 数据层已落库，完整产品化面板仍待增强 |
...
gantt
    ...
    section 2024
    MVP 核心闭环上线      :done, 2024-03-15, 1d
    宏观与通知能力补强     :done, 2024-07-01, 90d
    
    section 2025
    移动端主要页面接通     :done, 2025-01-01, 180d
    模拟交易与设置中心扩展  :done, 2025-07-01, 120d
    
    section 2026
    Neon/生产部署路径收敛  :done, 2026-01-01, 60d
    AI Provider 注册与代理优化 :done, 2026-03-01, 30d
```
...
| 验收项 | 标准 | 验收方式 |
|--------|------|----------|
| Web 主链路 | 登录、持仓、个股 AI、组合 AI、宏观雷达可独立跑通 | 手工回归 + 接口验证 |
| 后端运行稳定性 | FastAPI 启动即完成调度器拉起、内置 AI 注册、数据库连接初始化 | 启动验证 + 日志检查 |
| 数据一致性 | PostgreSQL/Neon 为唯一主数据源，关键分析与缓存结构可持续写入 | 数据库检查 |
| 大陆环境可用性 | 代理不可达时可自动清理代理；AkShare 相关域名支持直连 | 环境联调 |
| 多端一致性 | Web 为主体验，移动端覆盖核心查看/创建场景 | 多端冒烟测试 |

#### 21.2 迭代版本规划

##### 21.2.1 下一阶段重点一：Truth Tracker 产品化

**目标：把已落库的复盘数据真正转化为用户可见的信任资产**

| 功能 | 描述 | 当前判断 |
|------|------|----------|
| MAE/MFE追踪 | 记录诊断后的最大回撤/浮盈 | 数据层已具备，前台展示需增强 |
| 胜率统计 | 展示历史建议胜率 | 仍需补齐聚合视图与运营展示 |
| 历史切片 | 保存诊断时的数据快照 | 已有输入上下文快照能力 |
| 防穿越验证 | 验证数据时间有效性 | 适合纳入后续审计链路 |

##### 21.2.2 下一阶段重点二：模拟交易全生命周期

**目标：从“可创建”升级到“可完整管理、可复盘、可闭环”**

| 功能 | 描述 | 当前判断 |
|------|------|----------|
| 虚拟交易创建 | 创建虚拟订单 | 已具备 |
| 持仓中订单管理 | 编辑、平仓、状态流转 | 需增强 |
| 自动盯盘 | 自动监控止盈止损 | 后端已有基础，应继续产品化 |
| 策略回测与交易轨迹 | 验证策略有效性 | 仍需补齐完整展示 |

##### 21.2.3 下一阶段重点三：商业化与平台化闭环

**目标：把已存在的会员字段、API 能力和模型配置能力串成稳定商业体系**

| 功能 | 描述 | 当前判断 |
|------|------|----------|
| FREE/PRO 权益落地 | 将数据模型映射到真实功能门槛 | 适合近期推进 |
| 支付与订阅闭环 | 订阅购买、续费、降级 | 尚未在仓库内闭环 |
| API 商品化 | 对外开放诊断/行情能力 | 仍属中长期规划 |
| 多模型商业策略 | 公共模型 + BYOK + 供应商路由策略 | 已有底座，可继续扩展 |
