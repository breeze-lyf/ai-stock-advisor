...
- [backend/migrations/versions/877955ab3cd8_add_simulatedtrade_and_tradehistorylog_.py](file://backend/migrations/versions/877955ab3cd8_add_simulatedtrade_and_tradehistorylog_.py)
...
## 更新摘要
**所做更改**
- 新增通知系统章节，反映notification_logs表和notifications_enabled字段的完整实现
- 更新数据库架构概览，包含通知历史记录表的集成
- 增强用户配置管理，包含通知开关和精细化控制
- 新增通知历史记录API端点和前端集成
- 更新数据库完整性约束，包含通知表的索引优化
- 新增通知去重机制和智能推送策略
...
11. [分析报告系统](#分析报告系统)
12. [报告作用域管理](#报告作用域管理)
13. [数据库完整性约束](#数据库完整性约束)
14. [性能优化特性](#性能优化特性)
15. [数据库连接配置管理](#数据库连接配置管理)
16. [故障排除指南](#故障排除指南)
17. [总结](#总结)
...
**更新** 最新版本引入了完整的通知系统，包括notification_logs表、notifications_enabled字段和智能推送机制。系统现在支持多种通知类型，包括宏观预警、价格提醒、每小时摘要、每日报告和策略变更提醒，为用户提供全方位的智能提醒服务。
...
通知历史记录表用于持久化飞书机器人推送的信息，支持前端展示"提醒流"。
...
participant NotificationIndex as 通知索引
...
ReportScope->>Notification : 添加通知系统
Notification->>NotificationIndex : 添加索引优化
```
...
- [backend/migrations/versions/3e8869343958_add_notifications_enabled_to_user.py:24-25](file://backend/migrations/versions/3e8869343958_add_notifications_enabled_to_user.py#L24-L25)
- [backend/migrations/versions/877955ab3cd8_add_simulatedtrade_and_tradehistorylog_.py:34-36](file://backend/migrations/versions/877955ab3cd8_add_simulatedtrade_and_tradehistorylog_.py#L34-L36)
...
**通知系统迁移**：新增notification_logs表和notifications_enabled字段，实现完整的通知系统。

**通知索引优化迁移**：为notification_logs表添加了ticker和user_id索引，提升查询性能。
...
- [backend/migrations/versions/3e8869343958_add_notifications_enabled_to_user.py:1-37](file://backend/migrations/versions/3e8869343958_add_notifications_enabled_to_user.py#L1-L37)
- [backend/migrations/versions/877955ab3cd8_add_simulatedtrade_and_tradehistorylog_.py:1-65](file://backend/migrations/versions/877955ab3cd8_add_simulatedtrade_and_tradehistorylog_.py#L1-L65)
...
**通知精细化控制**：支持总通知开关和各类通知类型的单独控制

**章节来源**
- [backend/app/schemas/user_settings.py:1-24](file://backend/app/schemas/user_settings.py#L1-L24)
- [backend/app/models/user.py:47-80](file://backend/app/models/user.py#L47-L80)

## 通知系统

系统实现了完整的通知推送和历史记录管理功能，支持多种通知类型和智能去重机制。
...
+send_feishu_card(title, content, elements, color, webhook_url, msg_type, ticker, user_id) bool
+send_macro_alert(title, summary, heat_score, user_id, webhook_url) bool
+send_price_alert(ticker, name, current_price, target_price, is_stop_loss, user_id, webhook_url) bool
+send_hourly_summary(summary_text, count, sentiment, email, user_id, webhook_url) bool
+send_strategy_change_alert(ticker, name, old_strategy, new_strategy, change_reason, user_id, webhook_url) bool
}
class NotificationType {
<<enumeration>>
MACRO_ALERT
...
NotificationService --> NotificationLog : "持久化推送记录"
NotificationLog --> NotificationType
```
...
- 类型：MACRO_ALERT
- 内容：全球宏观热点探测和预警
- 特色：根据热度值显示不同颜色，支持自动去重

**价格提醒通知**：
- 类型：PRICE_ALERT
- 内容：止盈达成或止损警戒提醒
- 特色：区分止盈和止损，自动识别币种（USD/CNY）

**每小时摘要通知**：
- 类型：HOURLY_NEWS_SUMMARY
- 内容：财联社每小时新闻精要总结
- 特色：24小时去重逻辑，支持跳过严格去重的类型
...
**24小时去重**：默认情况下，同一用户、同一类型、同一股票的相同通知在24小时内不会重复发送

**跳过严格去重**：对于MACRO_SUMMARY和HOURLY_NEWS_SUMMARY类型，使用30分钟防重窗口，防止整点触发逻辑重叠

**状态检查**：仅对发送成功的通知进行去重检查，避免因失败而重复发送

**异常保护**：去重检查失败时不会阻塞通知发送，仅记录警告日志

### 通知历史记录

通知历史记录表用于持久化所有推送过的通知，支持前端展示"智能提醒流"：

**查询接口**：提供/get/api/v1/notifications/history端点，支持分页查询

**索引优化**：为ticker、user_id和created_at字段建立索引，提升查询性能

**前端集成**：前端组件AlertStream实时展示通知历史，支持自动刷新
...
NotificationLogs[NotificationLogs表]
end
Primary --> Stocks
Primary --> MarketData
Primary --> StockNews
...
**主键约束**：确保每张表的唯一标识符，如Stocks表的ticker、StockNews表的id、NotificationLogs表的id。

**外键约束**：维护表间关系的完整性，如MarketDataCache的ticker引用Stocks表，UserAIModel的user_id引用Users表。

**唯一约束**：防止重复数据的插入，如Users表的email、Portfolios表的(user_id, ticker)组合、AIModelConfigs表的key、UserAIModels表的(user_id, key)组合、NotificationLogs表的(id)。

**检查约束**：验证数据的有效性，如非空约束确保重要字段的完整性。

**索引约束**：优化查询性能，如StockNews表的ticker索引，AIModelConfigs表的key索引，NotificationLogs表的ticker、user_id和created_at索引。
...
**通知索引优化**：为notification_logs表建立了专门的查询索引，包括：
- ix_notification_logs_ticker：按股票代码查询
- ix_notification_logs_user_id：按用户ID查询
- ix_notification_logs_created_at：按时间查询
...
- [backend/app/main.py:275-294](file://backend/app/main.py#L275-L294)
...
**通知历史查询异常**：
- 症状：获取通知历史记录时出现错误
- 解决方案：检查notification_logs表是否存在，确认索引是否正确

**通知去重失效**：
- 症状：重复收到相同的通知
- 解决方案：检查去重逻辑的阈值设置，确认数据库连接正常
...
**通知系统故障**：
- 症状：通知无法发送或历史记录缺失
- 解决方案：检查Feishu Webhook配置，确认数据库表创建完成
...
**通知系统完整实现**：新增的notification_logs表、notifications_enabled字段和智能推送机制，实现了完整的通知生态系统

**通知历史记录**：支持前端展示"智能提醒流"，提供实时通知历史查询

**智能去重机制**：防止重复通知的智能去重逻辑，提升用户体验

**多类型通知支持**：支持宏观预警、价格提醒、每小时摘要、每日报告、策略变更等多种通知类型

**索引优化增强**：为notification_logs表建立了专门的查询索引，提升通知查询性能

该数据库模式为AI驱动的股票分析提供了坚实的基础，支持复杂的技术分析需求和实时数据处理要求。通过持续的演进和优化，系统能够适应不断变化的金融分析需求。最新的notification系统增强和数据库连接优化进一步提升了系统的灵活性、可靠性和性能，为用户提供更好的分析体验。