...
## 更新摘要
**变更内容**
- **新的工厂模式实现**：实现三层回退机制（AkShare -> AlphaVantage -> IBKR）
- **增强的代理配置优化**：通过TLS隔离实现线程级代理绕过，解决国内服务器访问限制
- **Tavily新闻提供商集成**：作为AI增强搜索功能的新闻源
- **智能回退机制**：当主要数据源不可用时自动切换到备用方案
...
1. **新的工厂模式实现**：实现三层回退机制（AkShare -> AlphaVantage -> IBKR）
2. **增强的代理配置优化**：通过TLS隔离实现线程级代理绕过，解决国内服务器访问限制
3. **Tavily新闻提供商集成**：作为AI增强搜索功能的新闻源
4. **智能回退机制**：当主要数据源不可用时自动切换到备用方案
5. **盘前市场状态识别**：新增盘前市场状态识别和处理机制
6. **多源API测试工具**：提供完整的US股票数据获取测试工具链

**更新** 该系统现已增强以下核心功能：
- **三层回退机制**：AkShare为主要数据源，AlphaVantage为备选，IBKR为最终备选
- **智能代理绕过**：通过TLS隔离实现线程级代理绕过，确保国内服务器稳定访问
...
- **三层回退策略**：通过AkShare -> AlphaVantage -> IBKR的回退机制确保数据获取稳定性
...
- [backend/app/services/market_providers/akshare.py:77-1021](file://backend/app/services/market_providers/akshare.py#L77-L1021)
- [backend/app/services/market_providers/ibkr.py:34-571](file://backend/app/services/market_providers/ibkr.py#L34-L571)
- [backend/app/services/market_providers/alpha_vantage.py:16-164](file://backend/app/services/market_providers/alpha_vantage.py#L16-L164)
...
Provider->>Provider : 三层回退机制：AkShare -> AlphaVantage -> IBKR
Provider->>Provider : 智能代理绕过
...
- [backend/app/services/market_providers/factory.py:11-59](file://backend/app/services/market_providers/factory.py#L11-L59)
...
CheckIBKR --> |否| UseAV{"AlphaVantage是否有API Key"}
UseAV --> |是| UseAV["使用AlphaVantage提供商"]
UseAV --> |否| UseAkShare["使用AkShare提供商"]
CheckSource --> |其他| UseAkShare
...
- **API Key检查**：AlphaVantage仅在配置API Key时使用
...
**章节来源**
- [backend/app/services/market_providers/akshare.py:1-1021](file://backend/app/services/market_providers/akshare.py#L1-L1021)
- [backend/app/services/market_providers/ibkr.py:1-571](file://backend/app/services/market_providers/ibkr.py#L1-L571)
- [backend/app/services/market_providers/alpha_vantage.py:1-164](file://backend/app/services/market_providers/alpha_vantage.py#L1-L164)
...
- **三层回退机制**：AkShare -> AlphaVantage -> IBKR的自动切换
- **模拟模式**：网络隔离时生成模拟数据
...
2. **三层回退机制**：AkShare为主要数据源，AlphaVantage为备选，IBKR为最终备选
3. **智能代理绕过**：通过TLS隔离实现线程级代理绕过，解决国内服务器访问限制
4. **Tavily新闻集成**：提供AI增强的新闻搜索功能，支持高质量市场新闻获取
5. **双市场支持**：AkShare提供商支持A股和美股两个市场的数据获取
6. **智能路由机制**：自动识别股票类型并选择最优数据源
7. **强大的容错能力**：多重故障转移机制确保系统稳定性
...
- **三层回退机制**：AkShare -> AlphaVantage -> IBKR的智能切换
- **智能代理绕过**：通过TLS隔离实现线程级代理绕过
...
该系统为AI智能投资顾问提供了坚实的数据基础设施，通过持续的功能增强和技术优化，能够更好地满足用户的投资决策需求。新增的三层回退机制、智能代理绕过功能、Tavily新闻集成以及Sina Finance API的深度集成，使得系统能够提供更加全面、准确和稳定的市场数据服务，特别是在盘前和盘后市场的数据获取方面具有显著优势。新的工厂模式实现确保了系统的高可用性和容错能力，为用户提供了一致可靠的数据服务体验。