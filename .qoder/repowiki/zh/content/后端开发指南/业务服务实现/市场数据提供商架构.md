...
## 更新摘要
**变更内容**
- **yfinance提供商移除**：完全移除YFinance提供商，统一使用AkShare作为默认数据源
- **AkShare提供商增强**：实现双市场支持（A股和美股）、智能路由机制、代理绕过功能
- **新增IBKR提供商**：支持IB/TWS连接，提供高质量的实时数据
- **工厂模式重构**：从多提供商模式改为以AkShare为主、IBKR为备选的智能路由
- **代理绕过机制**：通过TLS隔离实现线程级代理绕过，解决国内服务器访问限制
...
**更新** 该系统现已完全重构，移除了yfinance提供商并增强了AkShare提供商：

### 主要变更

1. **yfinance提供商移除**：完全移除YFinance提供商，统一使用AkShare作为默认数据源
2. **AkShare提供商增强**：实现双市场支持（A股和美股）、智能路由机制、代理绕过功能
3. **新增IBKR提供商**：支持IB/TWS连接，提供高质量的实时数据
4. **工厂模式重构**：从多提供商模式改为以AkShare为主、IBKR为备选的智能路由
...
**章节来源**
- [backend/app/services/market_providers/akshare.py:1-905](file://backend/app/services/market_providers/akshare.py#L1-L905)
...
**图表来源**
- [backend/app/services/market_providers/akshare.py:168-173](file://backend/app/services/market_providers/akshare.py#L168-L173)
